...
def _run_import_job(import_job, dry_run=True):
    ...
    if dry_run:
        ...
        if not result.invalid_rows:
            ...
            cols_error = lambda row: ''.join(['<mark>' + key + '</mark>' + '<br>' + row.error.message_dict[key][0] + '<br>' for key in row.error.message_dict.keys()])
            summary += '<tr><td>row</td>' + '<td>errors</td><td>' + '</td><td>'.join([f.column_name for f in resource.get_user_visible_fields()]) + '</td></tr>'
            summary += '<tr><td>' + '</td><td></td></tr><tr><td>'.join([str(row.number) + '</td><td>' + cols_error(row) + '</td><td>' + cols(row) for row in result.invalid_rows]) + '</tr>'
        summary += '</table>'
        summary += '</body>'
        summary += '</html>'
...
def run_export_job(pk):
    ...
    def TempFileSave(filename, content):
    ...
    def FileSave(filename, content):
        ...
        export_job.file.save(filename, django_file,save= True)
    ...
    class Resource(resource_class):
        ...
        def export_resource(self, *args, **kwargs):
            if self.row_number%2000 == 0 or self.row_number == 1:
    ...
    resource = Resource()
    while rows <= qs_len:
        sub_pk_list = pks[rows:rows+interval]
        queryset = export_job.get_queryset2(sub_pk_list)
        data = resource.export(queryset)
        serialized = format.export_data(data)
        if not format.is_binary():
            serialized = serialized.encode("utf8")
        rows= rows+interval +1
        TempFileSave(filename,serialized)

    FileSave(filename, ContentFile(serialized))
